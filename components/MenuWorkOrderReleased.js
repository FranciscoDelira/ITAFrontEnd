    ...
    const getWorkOrderReleased = async () => {
        try {
            const response = await axios.get(
                `http://192.168.100.93/ITABackEnd/public/api/workorder_released/${personaldata_id}`,
                {
                    headers: {
        ...
        <VStack height="100%" width="100%" space={4} _dark={{ bg: "tema.3" }} _light={{ bg: "tema.2" }}>
            ...
            <Box height="55%" w="90%" alignSelf="center" >
                {workOrderR.map((WOReleased) => (
                    <Box _dark={{ bg: "tema.2", color: "tema.3" }} _light={{ bg: "tema.3", color: "tema.2" }} _pressed={{ bg: 'tema.6' }} borderRadius="xl" height={"32%"} marginBottom="5%" padding={"3%"} key={WOReleased.id}>
                        <Text bold fontSize="xs" _dark={{ color: "tema.3" }} _light={{ color: "tema.2" }}>Folio: {WOReleased.id}</Text>
                        <Text bold fontSize="xs" _dark={{ color: "tema.3" }} _light={{ color: "tema.2" }}>Area solicitante: {WOReleased.department}</Text>
                        <Text bold fontSize="xs" _dark={{ color: "tema.3" }} _light={{ color: "tema.2" }}>Fecha de solicitud: {WOReleased.requestDate}</Text>
                        <HStack marginTop={"8%"} alignItems="center">
                            <Text bold fontSize="sm" _dark={{ color: "tema.10" }} _light={{ color: "tema.10" }}>{WOReleased.status.toUpperCase()}</Text>
