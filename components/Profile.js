...
function Profile ({ navigation, route }) {
    ...
    useEffect(() => {
        ...
        const getPersonalData = async () => {
          try {
            const response = await fetch(`http://192.168.8.103/ITABackEnd/public/api/personalData_show/${personaldata_id}`);
      ...
      useEffect(() => {
        ...
        const getUser = async () => {
            try {
                const response = await fetch(`http://192.168.8.103/ITABackEnd/public/api/user_show/${id}`);
                const data = await response.json();
