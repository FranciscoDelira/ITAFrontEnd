...
function VieMaiReq({navigation, route}) {
    ...
    useEffect(() => {
        ...
        const getMaintenanceReq = async () => {
            try{
                const response = await fetch(`http://10.0.9.8/ITABackEnd/public/api/maintenance_show/${requestId}`);
            ...
            <Box height="70%" w="90%" alignSelf="center">
                ...
                <HStack justifyContent={"center"}>
                    {requestMaintenance?.evidence1 && (
                        <Image
                        source={{
                            uri: `http://10.0.9.8/ITABackEnd/storage/app/${requestMaintenance?.evidence1}`,
                          }}
                            alt="image1"
                            size="md"
                        />
                    ...
                    {requestMaintenance?.evidence2 && (
                        <Image
                        source={{
                            uri: `http://10.0.9.8/ITABackEnd/storage/app/${requestMaintenance?.evidence2}`,
                          }}
                            alt="image2"
                            size="md"
                        />
                    ...
                    {requestMaintenance?.evidence3 && (
                        <Image
                        source={{
                            uri: `http://10.0.9.8/ITABackEnd/storage/app/${requestMaintenance?.evidence3}`,
                          }}
                            alt="image3"
                            size="md"
                        />
